# This module contains the Unparser class (copied from the Python 3.9.5 ast
# module).
# By sub-classing Unparser, you can target other languages which are close
# to Python, such as Cython.
import ast
import sys
import tokenize
from ast import (NodeVisitor, AsyncFunctionDef, FunctionDef, ClassDef, Module,
                 Expr, Constant, Tuple, Name, If, Starred)
...
class Unparser(NodeVisitor):
    ...
    def _str_literal_helper(
        ...
        possible_quotes = [q for q in possible_quotes
                           if q not in escaped_string]
    ...
    def visit_BoolOp(self, node):
        ...
        with self.require_parens(operator_precedence, node):
            s = f" {operator} "
            self.interleave(lambda: self.write(s),
                            increasing_level_traverse, node.values)
